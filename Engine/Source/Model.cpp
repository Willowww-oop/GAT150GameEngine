...
void Model::Draw(Renderer& renderer, const VectorTwo& position, float angle, float scale)
{
	...
	{
		VectorTwo p1 = m_points[i].Rotate(angle) * scale + position;
		VectorTwo p2 = m_points[i + 1].Rotate(angle) * scale + position;
