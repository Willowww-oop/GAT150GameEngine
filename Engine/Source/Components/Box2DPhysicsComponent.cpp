...
void Box2DPhysicsComponent::Initialize()
{
	...
	if (size.x == 0 && size.y == 0)
	{
		...
		size = VectorTwo{ textureComponent->source.w, textureComponent->source.h };
	}
...
void Box2DPhysicsComponent::ApplyForce(const VectorTwo& force)
{
...
void Box2DPhysicsComponent::SetPosition(const VectorTwo& position)
{
	m_rigidBody->SetTransform(position, m_rigidBody->GetAngle());
}

void Box2DPhysicsComponent::SetVelocity(const VectorTwo& velocity)
{
