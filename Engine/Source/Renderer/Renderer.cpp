...
{
	VectorTwo size = texture.lock()->GetSize();

	SDL_FRect destRect;
	destRect.x = x - size.x * 2;
	destRect.y = y - size.x * 2;
	destRect.w = size.x;
	destRect.h = size.y;
...
{
	VectorTwo size = texture.lock()->GetSize() * transform.scale;

	SDL_FRect destRect;
	destRect.x = transform.position.x - size.x / 2;
	destRect.y = transform.position.y - size.x / 2;
	destRect.w = size.x;
	destRect.h = size.y;
...
{
	VectorTwo size = VectorTwo{ source.w, source.h } * transform.scale;
